...
class SwinFusion(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=32,
                 embed_dim=64, Ex_depths=[1,1], Fusion_depths=[1,1], Re_depths=[1,1], 
                 Ex_num_heads=[4,4], Fusion_num_heads=[4,4], Re_num_heads=[4,4],
                 window_size=8, mlp_ratio=2, qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
                 use_checkpoint=False, upscale=1, img_range=1.0, upsampler='', resi_connection='1conv',
                 **kwargs):
        ...
        self.meanA = torch.zeros(1, 1, 1, 1)
        rgb_mean = (0.4488, 0.4371, 0.4040)
        self.meanB = torch.Tensor(rgb_mean).view(1, 3, 1, 1)
        ...
        self.conv_first1_A = nn.Conv2d(32, embed_dim, 3, 1, 1)
        self.conv_first1_B = nn.Conv2d(embed_dim, embed_dim, 3, 1, 1)
        self.conv_first2_A = nn.Conv2d(32, embed_dim, 3, 1, 1)
        self.conv_first2_B = nn.Conv2d(embed_dim, embed_dim, 3, 1, 1)
        ...
        if resi_connection == '1conv':
            ...
            self.conv_after_body_Fusion = nn.Conv2d(embed_dim*2, embed_dim, 3, 1, 1)
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            self.conv_last1 = nn.Conv2d(embed_dim, embed_dim, 3, 1, 1)
    ...
    def forward_features_Fusion(self, x, y):
        ...
        x_size = (x.shape[2], x.shape[3])
        x = self.lrelu(self.conv_first1_A(x))
        y = self.lrelu(self.conv_first2_A(y))
        x = self.patch_embed(x)
        ...
        x = self.relu(self.conv_after_body_Fusion(x))
    ...
    def forward_features_Re(self, x):        
        ...
        x = self.patch_unembed(x, x_size)
        # Convolution 
        x = self.relu(self.conv_last1(x))
        x = self.relu(self.conv_last2(x))
        x = self.relu(self.conv_last3(x))
        return x
    ...
    def forward(self, A, B):
        ...
        #print(self.mean_A[0,0,0,0])
        #print(self.mean_B[0,0,0,0])
        #self.mean = (self.mean_A + self.mean_B) / 2
        #print(self.mean[0,0,0,0])
        ...
        # Feedforward
        #x = self.forward_features_Ex_A(torch.cat([A,B],1))
        #y = self.forward_features_Ex_B(torch.cat([B,A],1))
        #x = self.forward_features_Fusion(x, y)

        x = self.forward_features_Fusion(A, B)
...
if __name__ == '__main__':
    ...
    model = SwinFusion(upscale=2, img_size=(height, width),
                   ...
                   embed_dim=60, num_heads=[6, 6, 6, 6], mlp_ratio=2, upsampler='fusion')
    print(model)
    print(height, width, model.flops() / 1e9)

    x = torch.randn((1, 1, height, width))
    y = torch.randn((1, 1, height, width))
    out = model(x,y)
    print(out.shape)
